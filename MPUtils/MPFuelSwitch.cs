...
namespace MaritimePack.engine
{
    public class MPresource
    {
        ...
        public MPresource(string _name, float _ratio)
        {
            name = _name;
        ...
        public MPresource(string _name)
        {
            name = _name;
            ID = _name.GetHashCode();
            ratio = 1f;
...
namespace MaritimePack.customization
{
    public class MPmodularTank
    {
        public List<engine.MPresource> resources = new List<engine.MPresource>();
    }
}
...
namespace MaritimePack.customization
{
    public class MPfuelSwitch : PartModule, IPartCostModifier
    {
        [KSPField]
        ...
        private List<MPmodularTank> tankList;
        ...
        public override void OnLoad(ConfigNode node)
        {
            ...
            {
                Debug.LogError("MPFuelSwitch: Error: Mass Discrepancy detected in part '" + part.name + "'.", part);
                Debug.LogError("MPFuelSwitch: BasePartMass: '" + basePartMass + "' PartMass: "+ part.mass);
            }
            configLoaded = true;
        ...
        private void initializeData()
        {
            ...
            {
                setupTankList(false);
                weightList = MPFunctions.parseDoubles(tankMass);
                tankCostList = MPFunctions.parseDoubles(tankCost);
        ...
        private void assignResourcesToPart(bool calledByPlayer)
        {
            ...
            if (HighLogic.LoadedSceneIsEditor)
            {
                for (int s = 0; s < part.symmetryCounterparts.Count; s++)
                {
                    ...
                    MPfuelSwitch symSwitch = part.symmetryCounterparts[s].GetComponent<MPfuelSwitch>();
                    if (symSwitch != null)
                    {
            ...
            //if (tweakableUI == null)
            //{
            //    tweakableUI = MPFunctions.FindActionWindow(part);
            //}
            //if (tweakableUI != null)
            //{
            //    tweakableUI.displayDirty = true;
        ...
        {
            tankList = new List<MPmodularTank>();
            ...
            for (int tankCount = 0; tankCount < resourceTankArray.Length; tankCount++)
            {
                ...
                for (int amountCount = 0; amountCount < resourceAmountArray.Length; amountCount++)
                {
                    ...
                    {
                        Debug.Log("MPfuelSwitch: error parsing resource amount " + tankCount + "/" + amountCount + ": '" + resourceTankArray[amountCount] + "': '" + resourceAmountArray[amountCount].Trim() + "'");
                    }
                }
            }
            ...
            {
                MPmodularTank newTank = new MPmodularTank();
                ...
                {
                    engine.MPresource newResource = new engine.MPresource(resourceNameArray[nameCount].Trim(' '));
        ...
        public override string GetInfo()
        {
            if (showInfo)
            {
                List<string> resourceList = MPFunctions.parseNames(resourceNames);
